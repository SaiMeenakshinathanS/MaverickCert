    ...
    <MainLayout>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          { label: 'Total Records', value: certs.length.toLocaleString(), color: 'text-indigo-400' },
        ].map((s, i) => (
      ...
      {/* Info banner */}
      <div className="flex items-center gap-3 bg-indigo-500/8 border border-indigo-500/20 rounded-xl px-4 py-3 mb-5">
        <span className="text-indigo-400 text-base flex-shrink-0">ℹ</span>
        <p className="text-xs text-slate-400">
          To generate new certificates, start a batch from the{' '}
          <button onClick={() => navigate('/batches')} className="text-indigo-400 hover:text-indigo-300 underline transition-colors font-medium">
            Batches page
          </button>
            ...
            {batchOptions.slice(0, 6).map(b => (
              <button key={b} onClick={() => setBatchFilter(b)}
                className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-all ${
                  batchFilter === b ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-slate-200 hover:bg-surface-muted border border-surface-border'
                }`}>
                {b === 'All' ? 'All batches' : b}
              </button>
            ))}
          ...
          <div className="flex gap-1">
            {['All', 'Generated', 'Sent', 'Pending'].map(s => (
              <button key={s} onClick={() => setStatusFilter(s)}
                className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-all ${
                  statusFilter === s ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-slate-200 hover:bg-surface-muted border border-surface-border'
                }`}>
                {s}
              </button>
            ))}
                    ...
                    <tr key={cert._id} className="table-row">
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-2.5">
                          <div className="w-7 h-7 bg-indigo-600/25 rounded-full flex items-center justify-center text-xs font-semibold text-indigo-300 flex-shrink-0">
                      ...
                      <td className="px-5 py-3.5">
                        <Badge variant={cert.emailStatus === 'SENT' ? 'info' : cert.emailStatus === 'FAILED' ? 'error' : 'default'}>
                          {cert.emailStatus}
                        </Badge>
                      </td>
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-2">
                          ...
                          {(cert.certificateStatus === 'GENERATED' || cert.certificateStatus === 'SENT') && (
                            <button
                              ...
                              className="text-xs text-indigo-400 hover:text-indigo-300 font-medium border border-indigo-500/20 px-2 py-1 rounded-lg hover:bg-indigo-500/10 transition-colors"
                            >
                              ↓ Download
                            </button>
                          )}
