...
import { Card, Badge, ProgressBar, AIBanner, StatCard, Pagination, usePaginatedData, Icon } from '../components/index.jsx';
...
export default function Dashboard() {
  ...
  const { page, pageSize, total, pageItems: pagedBatches, setPage, setPageSize } = usePaginatedData(batches, 5);

 
  return (
    <MainLayout>
      <div className="relative overflow-hidden rounded-xl border border-surface-border bg-gradient-to-r from-indigo-600/15 via-surface-card to-purple-600/10 px-6 py-5 mb-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold font-display text-lg shadow-lg shadow-indigo-500/30">
      ...
      <div className="grid grid-cols-5 gap-4 mb-5">
        {[
          { icon: 'certificate', accent: 'indigo', label: 'Total Certificates', value: stats.totalCertificates.toLocaleString(), sub: `${stats.emailsSent} emails sent`, color: 'text-white' },
          ...
          { icon: 'trendingUp', accent: 'purple', label: 'Share Rate', value: `${stats.completionRate}%`, sub: 'Overall', color: 'text-indigo-400' },
        ].map((stat, i) => (
      ...
      <AIBanner>
        ...
        {loading
          ...
          : `${batches.length} batch(es) active. ${stats.pendingUsers} employees awaiting LinkedIn posting. Engagement rate: ${stats.completionRate}%.`
        }
        <button
          onClick={() => navigate('/reports')}
          className="ml-2 text-indigo-300 hover:text-indigo-200 underline text-xs cursor-pointer transition-colors duration-150"
        >
          View full analysis ↗
        </button>
      </AIBanner>
      ...
      <Card>
        <div className="px-5 py-4 border-b border-surface-border flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="icon-chip bg-indigo-500/10 text-indigo-400 w-8 h-8">
              <Icon name="layers" className="w-4 h-4" />
            </div>
            <h2 className="font-semibold text-white text-sm">Recent Batches</h2>
          </div>
          {!loading && <Badge variant="indigo">{batches.length} total</Badge>}
        </div>
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <div className="p-8 text-center text-slate-500 text-sm">
              No batches yet.{' '}
              <button onClick={() => navigate('/create-batch')} className="text-indigo-400 hover:text-indigo-300 underline">
