...
export default function Template() {
    ...
    try {
      ...
      if (res) {
        if (res.mode === 'custom' || res.bgGradient) {
          ...
          setSelectedStyle(MAP[res.style] || selectedStyle);
  ...
  const bg = generatedData?.bgGradient || 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)';
  const accent = generatedData?.accentColor || '#f59e0b';
  const tc = generatedData?.textColor || '#ffffff';
            ...
            <>
                ...
                <div className="flex gap-2">
                  {[['quick', 'Quick Styles'], ['prompt', 'AI Prompt']].map(([m, l]) => (
                    <button key={m} onClick={() => { setTemplateMode(m); setShowVariations(false); }}
                      className={`flex-1 px-3 py-2 rounded-lg text-xs font-medium transition-all ${templateMode === m ? 'bg-indigo-600 text-white' : 'bg-surface-muted text-slate-400 hover:text-slate-200 border border-surface-border'}`}
                    >{l}</button>
                  ))}
                </div>
              </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {QUICK_STYLES.map(s => (
                      <button key={s} onClick={() => setSelectedStyle(s)}
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${selectedStyle === s ? 'bg-indigo-600 text-white' : 'bg-surface-muted text-slate-400 hover:text-slate-200 border border-surface-border'}`}
                      >{s}</button>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  {[['left', '⬅ Left'], ['center', '↔ Center'], ['right', 'Right ➡']].map(([a, l]) => (
                    <button key={a} onClick={() => setSignatureAlignment(a)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${signatureAlignment === a ? 'bg-indigo-600 text-white' : 'bg-surface-muted text-slate-400 hover:text-slate-200 border border-surface-border'}`}
                    >{l}</button>
                  ))}
                </div>
              </div>
              ...
              <div className="mb-4">
                ...
                <input type="file" accept="image/*" onChange={handleSigUpload}
                  className="block w-full text-xs text-slate-500 file:mr-4 file:py-1.5 file:px-3 file:rounded-lg file:text-xs file:font-medium file:bg-indigo-600 file:text-white hover:file:bg-indigo-500 file:border-0 file:cursor-pointer"
                />
        ...
        <div className="space-y-4">
          <Card className="p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                {!uploadedImage && !uploadedPDF && (
                  <span className="text-xs text-indigo-300 bg-indigo-500/10 border border-indigo-500/20 px-2 py-1 rounded font-medium">{selectedStyle}</span>
                )}
                {uploadedPDF && (
            ...
            {uploadedPDF ? (
              ...
              <div className="rounded-lg border border-slate-700 overflow-hidden">
                ...
                <div className="flex items-center justify-between px-3 py-2 bg-slate-800 border-b border-slate-700">
                  ...
                  <label className="cursor-pointer text-xs text-indigo-400 hover:text-indigo-300 border border-indigo-500/30 bg-indigo-500/10 hover:bg-indigo-500/20 px-2 py-1 rounded transition-all">
                    ↑ Replace file
            ...
            <Card className="p-5 slide-in">
              ...
              <div className="space-y-2">
                {variations.map((v, i) => (
                  ...
                  }} className="w-full text-left p-3 rounded-lg bg-surface-muted border border-surface-border hover:border-indigo-500 transition-all text-xs">
          ...
          <Card className="p-5">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {PLACEHOLDERS.map(p => (
                <span key={p} className="bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 text-xs px-2 py-1 rounded font-mono">{p}</span>
              ))}
            </div>
            <div className="flex gap-2">
              <Button variant="secondary" className="flex-1 justify-center">Edit template</Button>
              <Button onClick={() => navigate('/email')} className="flex-1 justify-center">Save & next →</Button>
            </div>
          </Card>
        </div>
      </div>
