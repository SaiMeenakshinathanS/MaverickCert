...
function CredentialsModal({ name, email, password, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="bg-[#1e2130] border border-surface-border rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-5 slide-in"
        onMouseDown={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 bg-indigo-500/20 border border-indigo-500/30 rounded-lg flex items-center justify-center text-xs font-bold text-indigo-300">
              {(name || '?')[0].toUpperCase()}
            </div>
            <div>
        ...
        <div className="space-y-3">
          <div>
            ...
            <div className="flex items-center gap-2 bg-indigo-500/10 border border-indigo-500/20 rounded-lg px-3 py-2">
              <p className="text-xs text-indigo-300 font-mono flex-1 break-all">{email}</p>
              <button
                onClick={() => navigator.clipboard?.writeText(email)}
                title="Copy email"
                className="text-slate-500 hover:text-indigo-400 transition-colors flex-shrink-0 text-[10px]"
              >
                ⧉
              </button>
            </div>
...
function DeleteDialog({ user, onConfirm, onCancel, loading }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm"
      onClick={onCancel}>
      <div className="bg-[#1a1d2e] border border-surface-border rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6 slide-in"
...
function CreateUserModal({ role, onSave, onCancel, existingEmails }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm"
      onClick={onCancel}>
      <div className="bg-[#1a1d2e] border border-surface-border rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6 slide-in"
        onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold text-white ${
              role === 'admin' ? 'bg-red-500/20 border border-red-500/30' : 'bg-indigo-500/20 border border-indigo-500/30'
            }`}>
              {roleLabel[0]}
            </div>
            <div>
              <h3 className="text-sm font-semibold text-white">Create New {roleLabel}</h3>
              <p className="text-xs text-slate-500">Role: <span className={role === 'admin' ? 'text-red-400' : 'text-indigo-400'}>{roleLabel}</span> (auto-assigned)</p>
            </div>
          </div>
      ...
      <Card>
        {loading ? (
          ...
          <div className="p-10 text-center text-slate-500 text-sm">
            No users yet. Use <span className="text-indigo-400">+ Add Admin</span> or <span className="text-indigo-400">+ Add Coordinator</span> to create one.
          </div>
        ) : (
          <>
            {/* Table header */}
            <div className="grid grid-cols-[2fr_2fr_1fr_1fr_auto] gap-4 px-5 py-3 border-b border-surface-border">
              {['User', 'Email', 'Role', 'Status', 'Actions'].map(h => (
                <p key={h} className="text-xs font-medium text-slate-500 uppercase tracking-wider">{h}</p>
              ))}
            </div>
            ...
            <div className="divide-y divide-surface-border">
              {users.map(user => {
                ...
                return (
                  <div key={user._id}
                    className="grid grid-cols-[2fr_2fr_1fr_1fr_auto] gap-4 items-center px-5 py-4 hover:bg-surface-muted/30 transition-colors">

                    {/* Avatar + name */}
                    <div className="flex items-center gap-3 min-w-0">
                      <div className={`w-8 h-8 ${user.avatarColor || 'bg-indigo-500'} rounded-full flex items-center justify-center text-xs font-semibold text-white flex-shrink-0`}>
                    ...
                    {(() => {
                      ...
                      return (
                        <Badge variant={role === 'admin' ? 'danger' : 'indigo'}>
