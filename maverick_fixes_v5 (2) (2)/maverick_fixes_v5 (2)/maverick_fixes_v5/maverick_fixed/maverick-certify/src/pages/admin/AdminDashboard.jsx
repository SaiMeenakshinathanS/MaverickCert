...
import { Card, PageHeader, Icon } from '../../components/index.jsx';
  ...
  const stats = [
    {
      ...
      icon: 'users', accent: 'bg-violet-500/15 text-violet-500',
    },
    {
      label: 'Total Batches',
      value: loading ? '—' : String(batches?.length ?? 0),
      icon: 'layers', accent: 'bg-blue-500/15 text-blue-500',
    },
    {
      label: 'Certificates Generated',
      value: loading ? '—' : (overview?.totalCertificates ?? 0).toLocaleString(),
      icon: 'certificate', accent: 'bg-emerald-500/15 text-emerald-500',
    },
  ];
        ...
        {stats.map((s, i) => {
          return (
            <div key={i} className="stat-card flex items-center gap-3">
              <div className={`icon-chip !w-11 !h-11 shrink-0 ${s.accent}`}>
