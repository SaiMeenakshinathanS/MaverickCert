...
export default function Reports() {
  ...
  // Recharts renders colors as raw SVG attributes, so it can't pick up the
  // app's CSS custom properties automatically the way Tailwind classes do.
  // Mirror the relevant tokens here so charts match the active theme instead
  // of always using the dark-mode-only colors they were originally built for.
  const chartColors = {
    axisTick: isDark ? '#9a8cba' : '#6b5d85', // var(--text-muted)
    legendText: isDark ? '#cbc0e0' : '#4b3f63', // var(--text-secondary)
  };
  ...
  // Pagination for the batch performance table
  const { page, pageSize, total, pageItems: pagedBatches, setPage, setPageSize } = usePaginatedData(filteredBatches, 10);
  ...
  // Each point includes a "week" backend label (e.g. "W21 '26") used for
  // grouping, plus weekStart (an ISO date) which we turn into a friendly
  // axis label like "May 25" — much easier to read at a glance than an
  // ISO week number.
  const trendData = (trends && trends.length > 0)
    ? trends.map(t => ({
        ...t,
        dateLabel: t.weekStart
          ? new Date(t.weekStart).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })
          : t.label,
      }))
    : overview ? [{ label: 'Now', dateLabel: 'Now', certs: overview.totalCertificates }] : [];
            ...
            {trendsLoading ? (
              ...
              <ResponsiveContainer width="100%" height={192}>
                <AreaChart data={trendData} margin={{ top: 5, right: 5, left: -20, bottom: 5 }}>
                  <defs>
                    <linearGradient id="certsFill" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="#7F77DD" stopOpacity={0.35} />
                      <stop offset="100%" stopColor="#7F77DD" stopOpacity={0} />
                    </linearGradient>
                  </defs>
                  <XAxis dataKey="dateLabel" tick={{ fill: chartColors.axisTick, fontSize: 10 }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fill: chartColors.axisTick, fontSize: 10 }} axisLine={false} tickLine={false} />
              ...
              <PieChart>
                ...
                <Legend
                  formatter={(value, entry) => (
                    <span style={{ color: chartColors.legendText, fontSize: '11px' }}>{value}: {entry.payload.value}</span>
                  )}
                />
                <Tooltip content={<ChartTooltip />} />
              </PieChart>
              ...
              {batchLoading ? (
                ...
                pagedBatches.map((b, i) => (
                  <tr key={b.batchId || i} className="table-row">
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2">
                        <div className="flex-1 bg-slate-700/40 rounded-full h-1.5 min-w-[60px]">
