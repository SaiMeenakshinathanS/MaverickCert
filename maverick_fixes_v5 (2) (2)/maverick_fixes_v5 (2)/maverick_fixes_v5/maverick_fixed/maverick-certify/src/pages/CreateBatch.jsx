...
function parsePromptToStyle(desc) {
  ...
  else if (d.includes('modern') || d.includes('dark') || d.includes('tech') || d.includes('emerald') || d.includes('teal')) result.style = 'modern';
...
export default function CreateBatch() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // FIX 2: If a custom .docx template is loaded, flush any edits and pass its HTML
      // so the backend can use the full document (header + footer + content) as the certificate body.
      let customDocxHtml = null;
      if (uploadedTmplDocx) {
        syncDocxEdits();
        customDocxHtml = docxEditorRef.current ? docxEditorRef.current.innerHTML : docxHtml;
      }
      ...
      const styleConfig = {
        ...
        // Pass the full docx HTML when a custom template is used
        ...(customDocxHtml ? { customDocxHtml } : {}),
      };
  ...
  // FIX: When entering Step 5 with a custom docx template, pre-populate email body
  // with the full docx HTML (header + content + footer preserved exactly as uploaded).
  useEffect(() => {
    if (step === 5 && uploadedTmplDocx && !emailGenerated) {
      syncDocxEdits();
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/email/generate-template`, {
        ...
        body: JSON.stringify({
          ...
          ...(emailTemplateMode === 'detailed' && emailPrompt.trim() ? { customPrompt: emailPrompt.trim() } : {}),
        }),
      });
      const result = await response.json();
      if (!response.ok || !result.success) throw new Error(result.message || 'Generation failed');
  ...
  const handleBulkSend = async () => {
    ...
    try {
      // FIX: when a custom docx template is active, flush edits and use the full
      // HTML (header + content + footer) as the email body so nothing is stripped.
      let finalEmailBody = emailBody;
      if (uploadedTmplDocx) {
        syncDocxEdits();
        finalEmailBody = (docxEditorRef.current ? docxEditorRef.current.innerHTML : docxHtml) || emailBody;
      }

      const response = await fetch(`${BACKEND_URL}/api/email/send/${form.batchId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ subject: emailSubject, body: finalEmailBody }),
      });
          ...
          <div className="slide-in space-y-5">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input type="date" value={form.trainingStartDate} onChange={update('trainingStartDate')} className="input-field text-teal-400" />
              </div>
              <div className="flex flex-col gap-1.5">
                ...
                <input type="date" value={form.trainingEndDate} onChange={update('trainingEndDate')} className="input-field text-teal-400" />
        ...
        {step === 2 && (
          <div className="slide-in space-y-5">
            <div>
              ...
              <div
                className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all duration-200 ${uploading ? 'border-teal-500/60 bg-teal-500/5' : uploadSuccess ? 'border-emerald-500/50 bg-emerald-500/5' : uploadError ? 'border-red-500/40 bg-red-500/5' : 'border-surface-border hover:border-teal-500/50 hover:bg-surface-muted'}`}
              ...
              >
                ...
                {uploading ? (
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm text-teal-400 font-medium">Validating...</p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-400 mb-1">Drag & drop your file here, or <span className="text-teal-400">click to browse</span></p>
                      ...
                      <table className="w-full text-xs">
                        ...
                        <tbody>
                          {validationResult.invalidData.slice(0, 10).map((invalid, i) => (
                            <tr key={i} className="border-b border-surface-border hover:bg-surface-muted transition-colors last:border-0">
            ...
            <div className="grid grid-cols-2 gap-6 mb-5">
              ...
              <div className="space-y-4 overflow-y-auto" style={{ maxHeight: 520 }}>
                <div className="flex items-center justify-between gap-2 mb-4">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className={`text-[11px] px-2 py-1 rounded font-medium transition-colors ${templateMode === 'quick' ? 'bg-teal-600 text-white hover:bg-teal-700' : 'bg-surface-muted border border-surface-border text-slate-400 hover:text-slate-300'}`}
                    >
                      Quick Mode
                    </button>
                    <button
                      onClick={() => setTemplateMode('ai')}
                      className={`text-[11px] px-2 py-1 rounded font-medium transition-colors ${templateMode === 'ai' ? 'bg-teal-600 text-white hover:bg-teal-700' : 'bg-surface-muted border border-surface-border text-slate-400 hover:text-slate-300'}`}
                    >
                      AI Mode
                    </button>
                  </div>
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {signatories.length < 2 && (
                      <button onClick={addSignatory}
                        className="text-xs text-teal-400 hover:text-teal-300 border border-teal-500/30 px-2 py-0.5 rounded transition-colors">
                        + Add Signatory
                      </button>
                    )}
                  </div>
                  <div className="space-y-3">
                    {signatories.map((sg, idx) => (
                      <div key={sg.id} className="bg-surface-muted border border-surface-border rounded-lg p-3 space-y-2">
                        ...
                        <div
                          className="border border-dashed border-surface-border rounded-lg p-2 text-center cursor-pointer hover:border-teal-500/50 transition-colors"
                          ...
                          {sg.imageUrl
                            ...
                            : <p className="text-[10px] text-slate-500">Upload signature image <span className="text-teal-400">(optional)</span></p>
                          }
                        </div>
                        {/* Position */}
                        <div>
                ...
                <button onClick={handleGenerateTemplate} disabled={tmplGenerating}
                  className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-semibold border transition-all duration-200 ${
                    ...
                    : tmplGenerating ? 'bg-teal-600/50 border-teal-500/30 text-teal-300 cursor-not-allowed'
                    : 'bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-500 hover:to-emerald-500 border-transparent text-white'
                  }`}>
                  {tmplGenerating ? <><span className="w-4 h-4 border-2 border-teal-300 border-t-transparent rounded-full animate-spin" />Generating...</>
              ...
              <div className="flex flex-col gap-3">
                ...
                {tmplGenerating ? (
                  ...
                  /* ── Docx A4 preview shell — shows the document exactly as uploaded ── */
            ...
            <div className="grid grid-cols-2 gap-5 mb-5">
              ...
              <div className="flex flex-col gap-3">
                ...
                {generating ? (
                  ...
                  /* Custom template — show docx preview so user sees the real certificate */
                  <div className={`bg-slate-900 p-3 rounded-lg max-h-[340px] overflow-y-auto ${genDone ? '' : 'opacity-40 pointer-events-none'}`}>
                    <div style={{ width:'100%', background:'#ffffff', boxShadow:'0 4px 16px rgba(0,0,0,0.4)', borderRadius:3 }}>
                      {docxHtml ? (
                        <div
                          className="p-6 text-slate-900 text-xs"
                          style={{ fontFamily:'Georgia, serif', lineHeight:1.7 }}
                          dangerouslySetInnerHTML={{ __html: docxHtml }}
                        />
                      ) : (
                        <div className="p-6 text-slate-400 text-xs text-center">Custom template loaded — content will be personalised per candidate</div>
                      )}
                    </div>
                    {genDone && <p className="text-[10px] text-emerald-400 text-center mt-2">✓ Custom template certificates generated</p>}
                  </div>
                ) : (
              ...
              <div className="space-y-4">
                <label className="flex items-center gap-2.5 cursor-pointer group">
                  <div className="relative">
                    ...
                    <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-all ${useDefaultTemplate ? 'bg-teal-600 border-teal-600' : 'border-surface-border group-hover:border-teal-500/50'}`}>
                ...
                {emailTemplateMode === 'detailed' && (
                  <div>
                    <label className="text-xs font-medium text-slate-400 uppercase tracking-wider block mb-1.5">Custom AI Prompt</label>
                    <textarea
                      rows={3}
                      value={emailPrompt}
                      onChange={e => setEmailPrompt(e.target.value)}
                      placeholder="Describe the tone, style, or specific details you want in the email… e.g. \'Formal tone, mention LinkedIn sharing, include manager CC\'"
                      className="input-field resize-none"
                    />
                  </div>
                )}
                ...
                <div>
                  <label className="text-xs font-medium text-slate-400 uppercase tracking-wider block mb-1.5">
                    Email Body {uploadedTmplDocx && <span className="text-emerald-400 normal-case">(custom template)</span>}
                  </label>
                  {emailGenerating ? (
                    ...
                    /* Custom template — show the rendered HTML so the user sees the real email */
                    <div className="rounded-lg border border-emerald-500/30 bg-white max-h-48 overflow-y-auto">
                      <div className="px-3 py-1.5 bg-emerald-500/10 border-b border-emerald-500/20 flex items-center justify-between">
                        <span className="text-[10px] text-emerald-400 font-medium">📄 {uploadedTmplDocx.filename}</span>
                        <span className="text-[10px] text-slate-400">Full template including header &amp; footer will be sent</span>
                      </div>
                      <div
                        className="p-4 text-slate-900 text-xs"
                        style={{ fontFamily:'Georgia, serif', lineHeight:1.7 }}
                        dangerouslySetInnerHTML={{ __html: emailBody || docxHtml || '' }}
                      />
                    </div>
                  ) : (
                ...
                <div>
                  ...
                  <div className="flex gap-2 mb-2">
                    <button onClick={handleGenerateLinkedIn} className="text-xs px-3 py-1.5 bg-teal-600 hover:bg-teal-700 rounded text-white font-medium transition-colors">📱 Generate LinkedIn Post</button>
                  </div>
                  ...
                  <div className="space-y-2 slide-in">
                    ...
                    {emailVariations.map((variant, idx) => (
                      <button
                        ...
                        className="w-full text-left p-2.5 rounded-lg bg-surface-muted border border-surface-border hover:border-teal-500 transition-all text-xs"
                      >
